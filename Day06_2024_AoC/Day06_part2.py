...
def make_one_move(guard_pos_and_dir, input):
    start = [guard_pos_and_dir[0], guard_pos_and_dir[1], guard_pos_and_dir[2]]
    move = 0
    ...
    while True:
        ...
        current_move = guard_pos_and_dir[2]
        if current_move in moves_made[guard_pos_and_dir[1]][guard_pos_and_dir[0]] :
            # remove the current move from the string
            moves_made[guard_pos_and_dir[1]][guard_pos_and_dir[0]] = moves_made[guard_pos_and_dir[1]][guard_pos_and_dir[0]].replace(current_move, "")
        else:
            return 1
